...
import pickle
from rhode_island_holdem import RhodeHand, get_deck

N_BUCKETS = 20
EQUITY_SAVE_NAME = 'rhode_flop_equities.pkl'
N_EQUITY_BINS = 50  # How many bins to divide the equity histogram into
ITERATIONS = 10    # How many iterations of k-means clustering to run
...
def compute_abstractions():
    ...
    means = np.zeros((N_BUCKETS, N_EQUITY_BINS))
    for i in range(means.shape[0]):
        random_hand = i #np.random.randint(matrix.shape[0])
